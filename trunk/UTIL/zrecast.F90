...
real :: R0 ! reduced gaz constant (Mars: R=191)
...
!
! AS 11/2014
! merging various versions of zrecast into one and only
...
character (len=64) :: timeunit ! to store the units for time axis
...
timeunit="  "
ierr=NF_GET_ATT_TEXT(infid,tmpvarid,'units',timeunit)
...
if (ierr.ne.NF_NOERR) then
 ...
 if (ztype.eq.1) then
   ...
   if (ierr.ne.NF_NOERR) then
     stop "Error: Could not define presnivs dimension either"
   endif
 else
   stop
 endif
...
if (ierr.ne.NF_NOERR) then
  ...
  if (ztype.eq.1) then
    ...
    if (ierr.ne.NF_NOERR) then
      stop "Error: Could not define presnivs variable either"
    endif
  else
    stop
  endif
...
ierr=NF_PUT_ATT_TEXT(outfid,time_varid,'units',len_trim(timeunit),timeunit)
if (ierr.ne.NF_NOERR) then
...
! if still no "controle" array nowhere, ask for planet name
...
if (have_ctl) then
  ...
  R0=1000.*8.314511/controle(8) ! controle(8)=mugaz = molar mass (g.mol-1) of atm.
else
   write(*,*) "No controle found." 
   write(*,*) "You need to tell me on which planet we are:"
   read(*,'(a20)') planet
   write(*,*) ""
   if (trim(planet).eq."titan") then
     a0=2575.E3 ! = radius of planet (m)
     g0=1.35 ! = gravity (m.s-2) at a0
     R0=296.9 ! = molecular gas constant
   else if (trim(planet).eq."venus") then
     a0=6051.E3 ! = radius of planet (m)
     g0=8.87 ! = gravity (m.s-2) at a0
     R0=191.4 ! = molecular gas constant
   else
     write(*,*) "init_planet_const: Failed to init ",trim(planet), &
                 " constants..."
     stop 
   endif
endif
...
!          surface altitudes" corresponding to GCM atmospheric levels
!==============================================================================
use planet_const, only : a0,g0,R0
implicit none
!==============================================================================
! Arguments:
!==============================================================================
