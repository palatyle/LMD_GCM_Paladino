...
SUBROUTINE dynredem0_p(fichnom,iday_end,phis)
  ...
  USE IOIPSL
#endif
  USE parallel_lmdz, ONLY: mpi_rank
...
!===============================================================================
  if (mpi_rank==0) then ! only the master reads input file
  ! fill dynredem_mod module variables
  modname='dynredem0_p'; fil=fichnom
...
!-------------------------------------------------------------------------------
  USE parallel_lmdz, ONLY: mpi_rank, gather_field
...
! Local variables:
  ...
  ! fill dynredem_mod module variables
  modname='dynredem1_p'; fil=fichnom
  ...
  IF (mpi_rank==0) THEN ! only the master writes restart file
