...
! if not using IOIPSL, we still need to use (a local version of) getin
  ...
  USE logic_mod, ONLY: tidal,purmats,ok_guide,read_start,iflag_phys,iflag_trac,	&
		ok_strato,ok_gradsfile,ok_limit,ok_etat0,moyzon_mu,moyzon_ch,	&
		fxyhypb,ysinus,read_orop
...
!Config         nouvelle parametrisation de l'eau liquide !
  ...
  IF (offline .AND. adjust) THEN
    WRITE(lunout,*)'WARNING : option offline does not work with adjust=y :'
    WRITE(lunout,*)'the files defstoke.nc, fluxstoke.nc ', &
                   'and fluxstokev.nc will not be created'
    WRITE(lunout,*) 'only the file phystoke.nc will still be created ' 
  END IF
...
! Ehouarn: at this point grossismx is undefined...
!      IF (use_filtre_fft .AND. grossismx /= 1.0) THEN
!        write(lunout,*)'WARNING !!! '
!        write(lunout,*)"the zoom in longitude grossismx=",grossismx,
!     &                 " is not compatible with an FFT filter",
