MODULE getincom
!-
!$Id: getincom.f90 4923 2020-02-12 10:28:41Z jgipsl $
!-
! This software is governed by the CeCILL license
! See IOIPSL/IOIPSL_License_CeCILL.txt
!---------------------------------------------------------------------
USE errioipsl, ONLY : ipslerr,ipsldbg,ipslout
USE stringop, &
 &   ONLY : nocomma,cmpblank,strlowercase,nocomment, COMMENT_TAG
!-
IMPLICIT NONE
!-
PRIVATE
PUBLIC :: getin_name, getin, getin_dump, getin_dump_para
!-
!!--------------------------------------------------------------------
...
!!
!! SUBROUTINE getin (targetname,ret_val)
!!
!! INPUT
!!
!! (C) targetname : Name of the variable
!!
!! OUTPUT
!!
...
!-
!!------------------------------------------------------------------------
!! Parallel version of getin_dump : user must give a fileprefix and a rank.
!! It may be usefull to verify that only the root proc is reading the input
!! def files.
!!
!!  SUBROUTINE getin_dump_para (fileprefix,rank)
!!
!! INPUT argument
!!
!! (C) fileprefix : allows the user to change the name of the file
!!                  in which the data will be archived
!! (I) rank : the rank of the parallel process (only 0-999 files will be created)
!!--------------------------------------------------------------------
!-
...
!-
  INTEGER,PARAMETER :: n_d_fmt=5,max_msgs=30
  CHARACTER(LEN=6),SAVE :: c_i_fmt = '(I5.5)'
!-
! The data base of parameters
!-
  INTEGER,PARAMETER :: memslabs=300
...
! keystatus = 3 : Some vector elements were taken from default
  INTEGER,PARAMETER :: nondefault=1, default=2, vectornondefault=3
!-
! keytype definition
! keytype = 1 : Integer
! keytype = 2 : Real
...
!-
  ...
  CHARACTER(LEN=300),SAVE,ALLOCATABLE,DIMENSION(:) :: c_mem
...
!=== INTEGER INTERFACE
!-
SUBROUTINE getinis (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
...
!-
  ...
  INTEGER :: pos,status=0,fileorig, size_of_in
!---------------------------------------------------------------------
!-
! Do we have this targetname in our database ?
!-
  CALL get_findkey (1,targetname,pos)
!-
  tmp_ret_val(1) = ret_val
  size_of_in = SIZE(tmp_ret_val)
  
!-
  IF (pos < 0) THEN
!-- Get the information out of the file
    CALL get_fil (targetname,status,fileorig,size_of_in,i_val=tmp_ret_val)
!-- Put the data into the database
    CALL get_wdb &
 &   (targetname,status,fileorig,1,i_val=tmp_ret_val)
  ELSE
!-- Get the value out of the database
    CALL get_rdb (pos,1,targetname,i_val=tmp_ret_val)
  ENDIF
  ret_val = tmp_ret_val(1)
!---------------------
END SUBROUTINE getinis
!===
SUBROUTINE getini1d (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!-- Put the data into the database
    CALL get_wdb &
 &   (targetname,status,fileorig,size_of_in,i_val=tmp_ret_val)
  ELSE
!-- Get the value out of the database
    CALL get_rdb (pos,size_of_in,targetname,i_val=tmp_ret_val)
  ENDIF
  ret_val(1:size_of_in) = tmp_ret_val(1:size_of_in)
!----------------------
END SUBROUTINE getini1d
!===
SUBROUTINE getini2d (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!=== REAL INTERFACE
!-
SUBROUTINE getinrs (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!-
  IF (pos < 0) THEN
!-- Get the information out of the file
    CALL get_fil (targetname,status,fileorig,size_of_in,r_val=tmp_ret_val)
!-- Put the data into the database
    CALL get_wdb &
 &   (targetname,status,fileorig,1,r_val=tmp_ret_val)
  ELSE
!-- Get the value out of the database
    CALL get_rdb (pos,1,targetname,r_val=tmp_ret_val)
  ENDIF
  ret_val = tmp_ret_val(1)
!---------------------
END SUBROUTINE getinrs
!===
SUBROUTINE getinr1d (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!-- Put the data into the database
    CALL get_wdb &
 &   (targetname,status,fileorig,size_of_in,r_val=tmp_ret_val)
  ELSE
!-- Get the value out of the database
    CALL get_rdb (pos,size_of_in,targetname,r_val=tmp_ret_val)
  ENDIF
  ret_val(1:size_of_in) = tmp_ret_val(1:size_of_in)
!----------------------
...
!===
SUBROUTINE getinr2d (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!=== CHARACTER INTERFACE
!-
SUBROUTINE getincs (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!-
  CHARACTER(LEN=:),ALLOCATABLE,DIMENSION(:) :: tmp_ret_val
  INTEGER :: pos,status=0,fileorig,size_of_in,ier
!---------------------------------------------------------------------
!-
...
!-
  IF (pos < 0) THEN
!-- Get the information out of the file
    CALL get_fil (targetname,status,fileorig,size_of_in,c_val=tmp_ret_val)
!-- Put the data into the database
    CALL get_wdb &
 &   (targetname,status,fileorig,1,c_val=tmp_ret_val)
  ELSE
!-- Get the value out of the database
    CALL get_rdb (pos,1,targetname,c_val=tmp_ret_val)
  ENDIF
  ret_val = tmp_ret_val(1)
!--
  DEALLOCATE(tmp_ret_val)
...
!===
SUBROUTINE getinc1d (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!-- Put the data into the database
    CALL get_wdb &
 &   (targetname,status,fileorig,size_of_in,c_val=tmp_ret_val)
  ELSE
!-- Get the value out of the database
    CALL get_rdb (pos,size_of_in,targetname,c_val=tmp_ret_val)
  ENDIF
  ret_val(1:size_of_in) = tmp_ret_val(1:size_of_in)
!----------------------
END SUBROUTINE getinc1d
!===
SUBROUTINE getinc2d (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!=== LOGICAL INTERFACE
!-
SUBROUTINE getinls (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!-
  IF (pos < 0) THEN
!-- Get the information out of the file
    CALL get_fil (targetname,status,fileorig,size_of_in,l_val=tmp_ret_val)
!-- Put the data into the database
    CALL get_wdb &
 &   (targetname,status,fileorig,1,l_val=tmp_ret_val)
  ELSE
!-- Get the value out of the database
    CALL get_rdb (pos,1,targetname,l_val=tmp_ret_val)
  ENDIF
  ret_val = tmp_ret_val(1)
!---------------------
END SUBROUTINE getinls
!===
SUBROUTINE getinl1d (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!-- Put the data into the database
    CALL get_wdb &
 &   (targetname,status,fileorig,size_of_in,l_val=tmp_ret_val)
  ELSE
!-- Get the value out of the database
    CALL get_rdb (pos,size_of_in,targetname,l_val=tmp_ret_val)
  ENDIF
  ret_val(1:size_of_in) = tmp_ret_val(1:size_of_in)
!----------------------
END SUBROUTINE getinl1d
!===
SUBROUTINE getinl2d (targetname,ret_val)
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=*) :: targetname
...
!-
SUBROUTINE get_fil (targetname,status,fileorig,nb_to_ret,i_val,r_val,c_val,l_val)
!---------------------------------------------------------------------
!- Subroutine that will extract from the file the values
!- attributed to the keyword targetname
!-
!- (C) targetname    : target for which we will look in the file
!- (I) status    : tells us from where we obtained the data
!- (I) fileorig  : index of the file from which the key comes
!- (I) nb_to_ret : size of output vector
...
!-
  INTEGER :: k_typ,it,pos,len_str,status_cnt,io_err
...
! Get the type of the argument
  ...
  IF ( (k_typ.NE.k_i) .AND. (k_typ.NE.k_r) .AND. (k_typ.NE.k_c) .AND. (k_typ.NE.k_l) ) THEN
...
!-- First try the target as it is
    CALL get_findkey (2,targetname,pos)
!---
!-- Another try
!---
    IF (pos < 0) THEN
      WRITE(UNIT=cnt,FMT=c_i_fmt) it
      CALL get_findkey (2,TRIM(targetname)//'__'//cnt,pos)
    ENDIF
!---
    IF (pos > 0) THEN
!-----
      ...
      IF (l_dbg) THEN
         WRITE(*,*) &
              &      'getin_fil : read key ',targetname,' from file ',fileorig,' has type ',k_typ
      ENDIF
!-----
!---- DECODE
!-----
...
!------ Is this the value of a compressed field ?
 ...
 &           'For key '//TRIM(targetname)//' we have a compressed field', &
...
! Now we set the status for what we found
  IF (def_beha) THEN
    status = default
    CALL ipslerr (1,'USING DEFAULT BEHAVIOUR FOR', &
 &   TRIM(targetname),' ',' ')
    WRITE(ipslout,*) 'USING DEFAULT BEHAVIOUR FOR ',TRIM(targetname)
  ELSE
    status_cnt = 0
    DO it=1,nb_to_ret
      IF (.NOT.found(it)) THEN
        ...
        IF      (status_cnt <= max_msgs) THEN
          WRITE (UNIT=ipslout,FMT='(" USING DEFAULTS : ",A)', &
 &               ADVANCE='NO') TRIM(targetname)
          IF (nb_to_ret > 1) THEN
            WRITE (UNIT=ipslout,FMT='("__")',ADVANCE='NO')
            WRITE (UNIT=ipslout,FMT=c_i_fmt,ADVANCE='NO') it
          ENDIF
          SELECT CASE (k_typ)
          CASE(k_i)
            WRITE (UNIT=ipslout,FMT=*) "=",i_val(it)
          CASE(k_r)
            WRITE (UNIT=ipslout,FMT=*) "=",r_val(it)
          CASE(k_c)
            WRITE (UNIT=ipslout,FMT=*) "=",c_val(it)
          CASE(k_l)
            WRITE (UNIT=ipslout,FMT=*) "=",l_val(it)
          END SELECT
...
!===
SUBROUTINE get_rdb (pos,size_of_in,targetname,i_val,r_val,c_val,l_val)
!---------------------------------------------------------------------
...
!-
  IF (key_tab(pos)%keytype /= k_typ) THEN
    CALL ipslerr (3,'get_rdb', &
 &   'Wrong data type for keyword '//TRIM(targetname), &
 &   '(NOT '//TRIM(c_vtyp)//')',' ')
  ENDIF
!-
 ...
 &      .OR.(key_tab(pos)%keymemlen /= 1) ) THEN
      CALL ipslerr (3,'get_rdb', &
 &     'Wrong compression length','for keyword '//TRIM(targetname),' ')
    ELSE
      SELECT CASE (k_typ)
      CASE(k_i)
  ...
  ELSE
    IF (key_tab(pos)%keymemlen /= size_of_in) THEN
      CALL ipslerr (3,'get_rdb', &
 &     'Wrong array length','for keyword '//TRIM(targetname),' ')
    ELSE
...
!===
SUBROUTINE get_wdb &
 &  (targetname,status,fileorig,size_of_in, &
...
! Fill out the items of the data base
  nb_keys = nb_keys+1
  key_tab(nb_keys)%keystr = targetname(1:MIN(LEN_TRIM(targetname),l_n))
  ...
  IF (l_dbg) THEN
     WRITE(*,*) &
 &     "get_wdb : nb_keys ",nb_keys," key_tab keystr   ",key_tab(nb_keys)%keystr,&
 &                                       ",keystatus   ",key_tab(nb_keys)%keystatus,&
 &                                       ",keytype     ",key_tab(nb_keys)%keytype,&
 &                                       ",keycompress ",key_tab(nb_keys)%keycompress,&
 &                                       ",keyfromfile ",key_tab(nb_keys)%keyfromfile,&
 &                                       ",keymemstart ",key_tab(nb_keys)%keymemstart
  ENDIF
...
!-
  INTEGER :: current
!-
  CHARACTER(LEN=:), ALLOCATABLE :: READ_str, NEW_str
  CHARACTER(LEN=300) :: last_key,key_str
  CHARACTER(LEN=n_d_fmt) :: cnt
  CHARACTER(LEN=10) :: c_fmt
  INTEGER :: nb_lastkey
!-
...
!-
  IF (l_dbg) THEN
    WRITE(ipslout,*) 'getin_readdef : Open file ',TRIM(filelist(current))
  ENDIF
!-
...
!---- If we have an empty line then the keyword finishes
      nb_lastkey = 0
      IF (l_dbg) THEN
        WRITE(ipslout,*) 'getin_readdef : Have found an emtpy line '
      ENDIF
    ENDIF
  ENDDO
!-
...
!-
  IF (l_dbg) THEN
     WRITE(ipslout,*) "nb_lines ",nb_lines,"nb_keys ",nb_keys
     WRITE(ipslout,*) "fichier ",fichier(1:nb_lines)
     WRITE(ipslout,*) "targetlist ",targetlist(1:nb_lines)
     WRITE(ipslout,*) "fromfile ",fromfile(1:nb_lines)
     WRITE(ipslout,*) "compline ",compline(1:nb_lines)
     WRITE(ipslout,*) '<-getin_readdef'
  ENDIF
!---------------------------
...
!-
  ...
  CHARACTER(LEN=*) :: key_str,NEW_str
  CHARACTER(LEN=*),INTENT(out) :: last_key
!-
! LOCAL
!-
  INTEGER :: len_str,blk,nbve,starpos
  CHARACTER(LEN=:), ALLOCATABLE :: tmp_str,new_key,mult
  CHARACTER(LEN=n_d_fmt) :: cnt
  CHARACTER(LEN=10) :: c_fmt
  LOGICAL :: l_dbg
!---------------------------------------------------------------------
...
!-
        ...
        nbve = nbve+1
        WRITE(UNIT=cnt,FMT=c_i_fmt) nbve
        compline(nb_lines)=compline(nb_lines-1)
!-
      ENDDO
!-
...
!-
  ...
  IF (l_dbg) THEN
     WRITE(ipslout,*) "getin_decrypt ->",TRIM(NEW_str), " : "
     WRITE(ipslout,*) "getin_decrypt ->", nb_lines,&
          & SIZE(fichier), &
          & SIZE(fromfile), &
          & SIZE(filelist)
     IF (nb_lines > 0) THEN
        WRITE(ipslout,*) "getin_decrypt ->", &
          & TRIM(fichier(nb_lines)), &
          & fromfile(nb_lines), &
          & TRIM(filelist(fromfile(nb_lines)))
        WRITE(ipslout,*) "                compline : ",compline(nb_lines)
        WRITE(ipslout,*) "                targetlist : ",TRIM(targetlist(nb_lines))
     ENDIF
     WRITE(ipslout,*) "                last_key : ",last_key
  ENDIF
!---------------------------
...
!---
 ...
 &  'getin_checkcohe : We will keep only the last value'
       CALL ipslerr (2,'getin_checkcohe','Found a problem on key ', &
 &                     TRIM(targetlist(line)), fichier(line)//" "//fichier(k))
      targetlist(line) = ' '
    ENDIF
  ENDDO
!-----------------------------
...
!===
!-
SUBROUTINE getin_readline(unitf, out_string, is_eof)
!---------------------------------------------------------------------
...
!-
9998 CONTINUE
  CALL ipslerr (3,'getin_readline','Error while reading file',' ',' ')
!-
7778 CONTINUE
  out_string = TRIM(dummy)
  is_eof = 1
    
END SUBROUTINE getin_readline
!-
! getin_skipafew: reads  
!-
SUBROUTINE getin_skipafew (unit,out_string,is_eof,nb_lastkey)
!---------------------------------------------------------------------
  USE ISO_FORTRAN_ENV,ONLY : IOSTAT_EOR,IOSTAT_END
!-
  IMPLICIT NONE
...
!-
     CALL getin_readline(unit, dummy, is_eof)
!-
...
!-
  ...
  CHARACTER(LEN=300),ALLOCATABLE :: tmp_char(:)
...
!---------------------------------------------------------------------
  IMPLICIT NONE
!-
  CHARACTER(LEN=300),ALLOCATABLE :: tmp_fic(:)
...
!-
  ...
  CHARACTER(LEN=20) :: c_tmp
  CHARACTER(LEN=300) :: tmp_str,used_filename
  INTEGER :: io_err
  LOGICAL :: l_dbg
!---------------------------------------------------------------------
  CALL ipsldbg (old_status=l_dbg)
...
!---
 ...
 &      'getin_dump : opens file : ',TRIM(used_filename),' if = ',if
      WRITE(ipslout,*) 'GETIN_DUMP : NUMBER OF KEYS : ',nb_keys
    ENDIF
    OPEN (UNIT=22,FILE=used_filename,iostat=io_err)
    IF (io_err /= 0) THEN
       CALL ipslerr (3,'getin_dump', &
            &   'Could not open file :',TRIM(used_filename), &
            &   '')
    ENDIF
!---
...
!-- of file which belong to it
    IF ( (if == 1).AND.(nbfiles > 1) ) THEN
      ...
      IF (l_dbg) THEN
         WRITE(ipslout,*) '# '
         WRITE(ipslout,*) '# This file is linked to the following files :'
         WRITE(ipslout,*) '# '
         DO iff=2,nbfiles
            WRITE(ipslout,*) 'INCLUDEDEF = ',TRIM(filelist(iff))
         ENDDO
         WRITE(ipslout,*) '# '
      ENDIF
    ENDIF
...
!------ Write some comments
 ...
 &          TRIM(key_tab(ikey)%keystr),' comes.'
        ...
        IF (l_dbg) THEN
           WRITE(ipslout,*) '#'
           WRITE(ipslout,*) '# Status of key ', ikey, ' : ',&
 &          TRIM(key_tab(ikey)%keystr),key_tab(ikey)%keystatus
        ENDIF
!-------
!------ Write the values
...
!---------------------------------------------------------------------
  IF ( rank < 1000 ) THEN
     isize=MIN(LEN_TRIM(fileprefix),75)
     usedfileprefix = fileprefix(1:isize)
     usedfileprefix((isize+1):(isize+1))='_'
     WRITE(usedfileprefix((isize+2):(isize+5)),'(I4.4)') rank
     IF (l_dbg) &
          WRITE(ipslout,*) 'Dump getin to file ',usedfileprefix
     CALL getin_dump(usedfileprefix)
  ENDIF
!------------------------
END SUBROUTINE getin_dump_para
