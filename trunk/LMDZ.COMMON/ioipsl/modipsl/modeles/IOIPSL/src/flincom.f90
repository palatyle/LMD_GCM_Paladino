MODULE flincom
!-
!$Id: flincom.f90 4863 2019-12-16 13:33:26Z jgipsl $
!-
...
!-
  ...
  USE errioipsl, ONLY : histerr, ipslout,ipslerr,ipsldbg
...
! Vertical axis
!-
  IF (l_dbg) WRITE(ipslout,*) 'flincre Vertical axis'
!-
...
! Time axis
!-
  IF (l_dbg) WRITE(ipslout,*) 'flincre time axis'
!-
...
! The longitude
!-
  IF (l_dbg) WRITE(ipslout,*) 'flincre Longitude axis'
!-
...
! The Latitude
!-
  IF (l_dbg) WRITE(ipslout,*) 'flincre Latitude axis'
!-
...
!-
  IF (l_dbg) WRITE(ipslout,*) 'flincre Variable'
!-
  iret = NF90_PUT_VAR (fid, levid, lev1(1:llm1))
!-
  IF (l_dbg) WRITE(ipslout,*) 'flincre Time Variable'
!-
  iret = NF90_PUT_VAR (fid, timeid, REAL(itaus(1:ttm1)))
!-
  IF (l_dbg) WRITE(ipslout,*) 'flincre Longitude'
!-
  iret = NF90_PUT_VAR (fid, lonid, lon1(1:iim1,1:jjm1))
!-
  IF (l_dbg) WRITE(ipslout,*) 'flincre Latitude'
!-
...
!---------------------------------------------------------------------
  ...
  IF (l_dbg) WRITE (*,*) ' iideb, iilen, jjdeb, jjlen, iim, jjm ', &
                           iideb, iilen, jjdeb, jjlen, iim, jjm
  IF (l_dbg) WRITE (*,*) ' lon ', lon(1,1), lon(iilen,jjlen)
  IF (l_dbg) WRITE (*,*) ' lat ', lat(1,1), lat(iilen,jjlen)
!-
  CALL flinopen_work &
    (filename, iideb, iilen, jjdeb, jjlen, do_test, &
     iim, jjm, llm, lon, lat, lev, ttm, itaus, date0, dt, fid_out)
...
!---------------------------------------------------------------------
  ...
  jjmc=jjm
  CALL flinopen_work &
    (filename, 1, iimc, 1, jjmc, do_test, &
     iim, jjm, llm, lon, lat, lev, ttm, itaus, date0, dt, fid_out)
!-------------------------
...
!-
  IF (l_dbg) &
    WRITE(ipslout,*) 'OUT OF flininfo :',tmp_iim,tmp_jjm,tmp_llm,tmp_ttm
!-
  fid = ncids(fid_out)
!-
...
!-
  IF (l_dbg) &
    WRITE(ipslout,*) 'flininfo 2.2 We test if we have to test : ',do_test
!-
  IF (do_test) THEN
    IF      (iim /= tmp_iim) THEN
...
!---
    IF (l_dbg) WRITE(ipslout,*) 'flinopen 2.3 Else sizes are returned'
!---
    iim = tmp_iim
    jjm = tmp_jjm
    llm = tmp_llm
...
!-
  IF (l_dbg) WRITE(ipslout,*) 'flinopen 3.0 we are realy talking'
!-
  IF (do_test) THEN
!---
...
!---
    IF (l_dbg) &
      WRITE(ipslout,*) 'from file lon first and last, modulo 360. ', &
        x_first, x_last, MODULO(x_first,360.), MODULO(x_last,360.)
    IF (l_dbg) &
      WRITE(ipslout,*) 'from model lon first and last, modulo 360. ', &
...
!---
    IF (l_dbg) WRITE(ipslout,*) 'flinopen 4.0 extracting the coordinates'
!---
    CALL flinfindcood (fid_out, 'lon', vid, nbdim)
    IF (nbdim == 2) THEN
...
!-
  IF (l_dbg) WRITE(ipslout,*) 'flinopen 5.0 Get time'
!-
  IF (ttm > 0) THEN
!---
...
!---
    ...
    DEALLOCATE(vec_tmp)
!---
    IF (l_dbg) WRITE(ipslout,*) 'flinopen 5.1 Times ',itaus
!---
...
!--
    ...
    IF (gdtt_id > 0) THEN
      ...
      IF (l_dbg) &
        WRITE(ipslout,*) 'flinopen 5.1 gdtt_id year0 ... date0 ', &
...
!-----
      IF (l_dbg) &
        WRITE(ipslout,*) 'flinopen 5.1 gdtmaf_id year0 ... date0 ', &
                   year0, month0, day0, sec0, date0
    ELSE IF (old_id > 0) THEN
...
!-----
  ...
  ENDIF
!-
  IF (l_dbg) WRITE(ipslout,*) 'flinopen 6.0 File opened', date0, dt
!---------------------------
...
!-
  ...
  ENDDO
!-------------------------
  IF (ALLOCATED(buff_tmp)) DEALLOCATE(buff_tmp)
...
!-
  ...
  INTEGER, INTENT(IN) :: itau_dep, itau_fin, iideb, iilen, jjdeb, jjlen
  REAL, INTENT(OUT) :: var(:)
!-
! LOCAL
!-
...
!-
  IF (nb_atts > 0) THEN
     IF (l_dbg) THEN
      WRITE(ipslout,*) 'flinget_mat : attributes for variable :'
    ENDIF
  ENDIF
  DO i=1,nb_atts
    ...
    ELSE
      ...
      iret = NF90_GET_ATT (fid, vid, att_n, tmp_n)
        IF (l_dbg) THEN
        WRITE(ipslout,*) '   ',TRIM(att_n),' : ',TRIM(tmp_n)
      ENDIF
...
!---
    IF (ALLOCATED(var_tmp)) THEN
      IF (SIZE(var_tmp) < PRODUCT(w_len(:),mask=(w_len>1))) THEN
         DEALLOCATE(var_tmp)
         ALLOCATE(var_tmp(PRODUCT(w_len(:),mask=(w_len>1))))
      ENDIF
    ELSE
      ALLOCATE(var_tmp(PRODUCT(w_len(:),mask=(w_len>1))))
    ENDIF
...
!-
  ...
  INTEGER :: attlen, attnum
  INTEGER :: ndims, nb_atts
  INTEGER,DIMENSION(NF90_MAX_VAR_DIMS) :: dimids
  LOGICAL :: var_exists
!-
  LOGICAL :: l_dbg
...
!---------------------------------------------------------------------
  ...
  IF (l_dbg) THEN
    WRITE (ipslout,*) 'flinget_scal in file with id ',fid_in
  ENDIF
!-
  fid = ncids(fid_in)
  iret = NF90_INQUIRE_ATTRIBUTE(fid, NF90_GLOBAL, varname, len=attlen, attnum=attnum)
!-
! 1.0 Reading a global attribute
!-
  IF ( iret == nf90_noerr ) THEN
     !
     ! This seems to be a Global attribute
     !
     iret = NF90_GET_ATT (fid, NF90_GLOBAL, varname, var)
  ELSE
     !
     ! If there was an error on the test for a global attribute it
     ! is perhaps a scalar variable.
     !
     vid = -1
     iret = NF90_INQ_VARID (fid, varname, vid)
     !
     ...
     ENDIF
     IF (l_dbg) THEN
        WRITE(ipslout,*) "Reading a Scalar value for varibale ", varname," It has value ", var
     ENDIF
  ENDIF
!-
!---------------------------
