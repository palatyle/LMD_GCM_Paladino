...
!!         call iophys_ini
         ...
         write(*,*) "output_dir in writediagfi:",trim(output_dir)
         ierr = NF_CREATE(trim(output_dir), NF_CLOBBER, nid)
...
!#endif
     ...
     .          "days since 0000-00-0 00:00:00")
         ...
         if (is_master) then
           ...
           ierr = NF_OPEN(trim(output_dir),NF_WRITE,nid)
         endif ! of if (is_master)
