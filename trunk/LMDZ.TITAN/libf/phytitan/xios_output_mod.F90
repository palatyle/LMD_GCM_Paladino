...
!                                is_sequential, is_south_pole_dyn
  ...
  USE comchem_h, ONLY: preskim, preskim_tot, zlaykim, zlaykim_tot
...
!$OMP MASTER
    ...
    IF (prt_level>=10) WRITE(lunout,*) "initialize_xios_output: call xios_set_axis_attr for preskim"
    CALL xios_set_axis_attr("preskim", n_glo=size(preskim), value=preskim,&
                            unit="Pa",positive="down")
                          
    IF (prt_level>=10) WRITE(lunout,*) "initialize_xios_output: call xios_set_axis_attr for preskim_tot"
    ...
    !CALL xios_set_axis_attr("zlaykim", n_glo=size(preskim), value=preskim,&
    !                        unit="km",positive="up")
    !                      
    !IF (prt_level>=10) WRITE(lunout,*) "initialize_xios_output: call xios_set_axis_attr for zlaykim_tot"
    !CALL xios_set_axis_attr("zlaykim_tot", n_glo=size(zlaykim_tot), value=zlaykim_tot,&
    !                        unit="km",positive="up")
